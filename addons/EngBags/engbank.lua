...
EngBank_ConfigOptions_Default          = {
    ...
    { -- Window Columns
        ...
        { ["type"] = "Text", ["ID"] = 2, ["width"] = 0.1, ["color"] = { 0, 1, 0.5 }, ["align"] = "right", ["text"] = EngBags_MAXCOLUMNS_MIN },
        {
            ...
            ["ID"] = 1,
            ["width"] = 0.4,
            ["minValue"] = EngBags_MAXCOLUMNS_MIN,
            ["maxValue"] = EngBags_MAXCOLUMNS_MAX,
            ["valueStep"] = 1,
    ...
    { -- Button Size
        ...
        { ["type"] = "Text", ["ID"] = 2, ["width"] = 0.1, ["color"] = { 0, 1, 0.5 }, ["align"] = "right",     ["text"] = EngBags_BUTTONSIZE_MIN },
        {
            ...
            ["ID"] = 1,
            ["width"] = 0.4,
            ["minValue"] = EngBags_BUTTONSIZE_MIN,
            ["maxValue"] = EngBags_BUTTONSIZE_MAX,
            ["valueStep"] = 1,
            ["defaultValue"] = function()
    ...
    { -- Font Size / item count
        ...
        { ["type"] = "Text", ["ID"] = 2, ["width"] = 0.1, ["color"] = { 0, 1, 0.5 }, ["align"] = "right",              ["text"] = EngBags_FONTSIZE_MIN },
        {
            ...
            ["ID"] = 1,
            ["width"] = 0.4,
            ["minValue"] = EngBags_FONTSIZE_MIN,
            ["maxValue"] = EngBags_FONTSIZE_MAX,
            ["valueStep"] = 1,
            ["defaultValue"] = function()
        ...
        table.insert(EngBank_ConfigOptions,
            {
                ...
                },
                {
                    ...
                    ["param1"] = key,
                    ["param2"] = 1,
                    ["defaultValue"] = EngBank_Config_GetItemSearchListLower,
                    ["func"] = EngBank_Config_AssignItemSearchListUpper
                },
                {
                    ["type"] = "Edit",
                    ["ID"] = 2,
                ...
                },
                {
                    ...
                    ["param1"] = key,
                    ["param2"] = 3,
                    ["defaultValue"] = EngBank_Config_GetItemSearchList,
                    ["func"] = EngBank_Config_AssignItemSearchList
                },
                {
                    ["type"] = "Edit",
                    ["ID"] = 4,
...
function EBank_SetDefault(varname, defaultvalue, resetversion, cleanupfunction, cleanup_param1, cleanup_param2)
    ...
    elseif (EngBankConfig[varname .. "__version"] < resetversion) then
        ...
        EngBags_Print(
        varname .. " was reset to it's default value.  Changed from '" .. orig_value .. "' to " .. EngBankConfig
        [varname], 1, 0, 0);
        EngBankConfig[varname] = defaultvalue;
    end
...
function EBags_SetDefault(varname, defaultvalue, resetversion, cleanupfunction, cleanup_param1, cleanup_param2)
    ...
    elseif (EngBankConfig[varname .. "__version"] < resetversion) then
        EngBags_PrintDEBUG("old version: " .. EngBankConfig[varname .. "__version"] .. ", resetversion: " .. resetversion);
        EngBags_Print(
        varname .. " was reset to it's default value.  Changed from '" .. orig_value .. "' to " .. EngBagsConfig
        [varname], 1, 0, 0);
        EngBagsConfig[varname] = defaultvalue;
    end
...
function EngBank_SetDefaultValues(re)
    ...
    EBank_SetDefault("bar_sort_" .. EngBankConfig["putinslot--EMPTY_SLOTS"], EngBags_SORTBYNAMEREV, 2 + re,
        EngBags_NumericRange, EngBags_SORTLOWESTVALUE, EngBags_SORTHIGHESTVALUE);
    EBank_SetDefault("bar_sort_" .. EngBankConfig["putinslot--HEALINGPOTION"], EngBags_SORTBYNAMEREV, 2 + re,
        EngBags_NumericRange, EngBags_SORTLOWESTVALUE, EngBags_SORTHIGHESTVALUE);
    EBank_SetDefault("bar_sort_" .. EngBankConfig["putinslot--MANAPOTION"], EngBags_SORTBYNAMEREV, 2 + re,
        EngBags_NumericRange, EngBags_SORTLOWESTVALUE, EngBags_SORTHIGHESTVALUE);
    EBank_SetDefault("bar_sort_" .. EngBankConfig["putinslot--TRADEGOODS"], EngBags_SORTBYNAMEREV, 2 + re,
        EngBags_NumericRange, EngBags_SORTLOWESTVALUE, EngBags_SORTHIGHESTVALUE);

    for i = 1, EngBags_MAX_BARS do
        EBank_SetDefault("bar_sort_" .. i, EngBags_SORTBYNAME, 2 + re, EngBags_NumericRange, EngBags_SORTLOWESTVALUE,
            EngBags_SORTHIGHESTVALUE);
...
function EngBank_Update_BagSlots()
    ...
    if ((not full) and (EngBankConfig["hide_purchase button"] == 0)) then
        MoneyFrame_Update("EngBank_SlotCostFrame", cost or 0);
    else
...
function EngBank_StopMoving(frame)
    if (frame.isMoving) then
        ...
        EngBags_PrintDEBUG("new position:  top=" ..
        EngBankConfig["frameTOP"] ..
        ", bottom=" ..
        EngBankConfig["frameBOTTOM"] .. ", left=" .. EngBankConfig["frameLEFT"] ..
        ", right=" .. EngBankConfig["frameRIGHT"]);
    end
end
            ...
            if (bagNumSlots > 0) then
                ...
                for slotnum = 1, bagNumSlots do
                    ...
                    if (itm["itemlink"] ~= nil) then
                        ...
                        itm["itemname"], itm["itemlink2"], itm["itemRarity"], itm["itemMinLevel"], itm["itemtype"], itm["itemsubtype"], itm["itemstackcount"], itm["itemloc"] =
                        GetItemInfo(itm["itemlink"]);
                        itm["texture"], itm["itemcount"], itm["locked"], itm["quality"], itm["readable"] =
        ...
        if (EngBankConfig["show_Bag" .. bagnum] == 1) then
            ...
            if (bagNumSlots > 0) then
                for slotnum = 1, bagNumSlots do
                    ...
                    table.insert(EngBank_bar_positions[EngBank_item_cache[bagnum][slotnum]["bar"]],
                        { ["bagnum"] = bagnum, ["slotnum"] = slotnum });
                end
            end
        end
    end
...
function EngBank_UpdateButton(itemframe, itm)
    ...
    itemframe_font:SetPoint("BOTTOMRIGHT", itemframe:GetName(), "BOTTOMRIGHT", 0 - EngBank_BUTTONFONTDISTANCE_X,
        EngBank_BUTTONFONTDISTANCE_Y);
    ...
    itemframe_stock:SetPoint("TOPLEFT", itemframe:GetName(), "TOPLEFT", (EngBank_BUTTONFONTDISTANCE_X / 2),
        0 - EngBank_BUTTONFONTDISTANCE_Y);
    ...
    if (not itm["itemlink"]) then
        if (EngBank_edit_mode == 1) then
            ...
            if (itm["barClass"] ~= nil) then
                if (EngBank_edit_selected ~= "") then
                    GameTooltip:AddLine(
                    "|cFF00FF7FLeft click to move catagory |r" .. EngBank_edit_selected .. "|cFF00FF7F to bar |r" .. bar,
                        1, 0.25, 0.5);
                else
                    GameTooltip:AddLine("|cFF00FF7FLeft click to select catagory to move:|r " .. itm["barClass"], 1, 0.25,
                        0.5);
...
--]]
    ...
    if (EngBank_edit_mode == 1) then
        ...
        if (itm["barClass"] ~= nil) then
            if (EngBank_edit_selected ~= "") then
                GameTooltip:AddLine(
                "|cFF00FF7FLeft click to move catagory |r" .. EngBank_edit_selected .. "|cFF00FF7F to bar |r" .. bar, 1,
                    0.25, 0.5);
            else
...
function EngBank_SlotTargetButton_OnEnter()
    ...
    if (EngBank_edit_mode == 1) then
        ...
        if (EngBank_edit_selected ~= "") then
            ...
            GameTooltip:ClearLines();
            GameTooltip:AddLine(
            "|cFF00FF7FLeft click to move catagory |r" .. EngBank_edit_selected .. "|cFF00FF7F to bar |r" .. bar, 1, 0.25,
                0.5);
            GameTooltip:Show();
            return;
        else
...
function EngBank_SetNewColor(previousValues)
    ...
    if (UIDROPDOWNMENU_MENU_VALUE ~= nil) then
        if (r ~= nil) then
            EngBankConfig["bar_colors_" .. (UIDROPDOWNMENU_MENU_VALUE["bar"]) .. "_" .. (UIDROPDOWNMENU_MENU_VALUE["element"]) .. "_r"] =
            r;
        end
        if (g ~= nil) then
            EngBankConfig["bar_colors_" .. (UIDROPDOWNMENU_MENU_VALUE["bar"]) .. "_" .. (UIDROPDOWNMENU_MENU_VALUE["element"]) .. "_g"] =
            g;
        end
        if (b ~= nil) then
            EngBankConfig["bar_colors_" .. (UIDROPDOWNMENU_MENU_VALUE["bar"]) .. "_" .. (UIDROPDOWNMENU_MENU_VALUE["element"]) .. "_b"] =
            b;
        end
        if (opacity ~= nil) then
            EngBankConfig["bar_colors_" .. (UIDROPDOWNMENU_MENU_VALUE["bar"]) .. "_" .. (UIDROPDOWNMENU_MENU_VALUE["element"]) .. "_a"] =
            opacity;
        end
        EngBank_window_update_required = EngBank_MANDATORY;
        EngBank_UpdateWindow();
...
function EngBank_RightClick_DeleteItemOverride()
    ...
    if ((bagnum ~= nil) and (slotnum ~= nil)) then
        ...
        if ((itm["itemlink_override_key"] ~= nil) and (EngBankConfig["item_overrides"][itm["itemlink_override_key"]] ~= nil)) then
            EngBankConfig["item_overrides"] = EngBags_Table_RemoveKey(EngBankConfig["item_overrides"],
                itm["itemlink_override_key"]);
            HideDropDownMenu(1);
            EngBank_resort_required = EngBank_MANDATORY;
            -- resort will force a window redraw as well
            EngBank_UpdateWindow();
...
function EngBank_frame_RightClickMenu_populate(level)
    ...
    if (EngBank_RightClickMenu_mode == "item") then
        ...
        if (level == 1) then
            ...
            info = { ["text"] = "Current Catagory: " .. itm["barClass"], ["notClickable"] = 1, ["isTitle"] = 1,
                ["notCheckable"] = nil };
            UIDropDownMenu_AddButton(info, level);
                ...
                for key, value in itm do
                    if (value == nil) then
                        ...
                        if ((type(value) == "number") or (type(value) == "string")) then
                            info = { ["text"] = "|cFFFF7FFF" .. key .. "|r = |cFF007FFF" .. value .. "|r",
                                ["notClickable"] = 1 };
                            UIDropDownMenu_AddButton(info, level);
                        else
                            ...
                            for key2, value2 in value do
                                info = { ["text"] = "    |cFFFF7FFF[" .. key2 .. "]|r = |cFF338FFF" .. value2 .. "|r",
                                    ["notClickable"] = 1 };
                                UIDropDownMenu_AddButton(info, level);
                            end
                        end
...
function EngBank_MoveAndSizeFrame(frameName, childAttachPoint, parentFrameName, parentAttachPoint, xoffset, yoffset,
                                  width, height)
...
function EngBank_AssignButtonsToFrame(barnum, currentbutton, frame, width, height)
    ...
    if (table.getn(EngBank_bar_positions[barnum]) > 0) then
        for position = 1, table.getn(EngBank_bar_positions[barnum]) do
            ...
            EngBank_buttons["EngBank_frame_Item_" .. currentbutton] = { ["bar"] = barnum, ["position"] = position,
                ["bagnum"] = bagnum, ["slotnum"] = slotnum };
    ...
    if (EngBank_window_update_required > EngBank_NOTNEEDED) then
        ...
        for barnum = 1, EngBags_MAX_BARS, 3 do
            ...
            if (calc_dat["height"] == 0) then
                ...
                available_width = (EngBankConfig["maxColumns"] * EngBags_BUTTONFRAME_WIDTH) +
                (10 * EngBankConfig["frameXSpace"]);
                ...
                if (calc_dat["second_width"] > 0) then
                    ...
                    EngBank_MoveAndSizeFrame("EngBank_frame_bar_" .. (barnum + 1), "BOTTOMRIGHT",
                        "EngBank_frame", "BOTTOMRIGHT",
                        0 -
                        (mid_point - (((calc_dat["second_width"] * EngBags_BUTTONFRAME_WIDTH) + (2 * EngBankConfig["frameXSpace"])) / 2)),
                        cur_y,
                ...
                if (calc_dat["third_width"] > 0) then
                    EngBank_MoveAndSizeFrame("EngBank_frame_bar_" .. (barnum + 2), "BOTTOMRIGHT",
                        "EngBank_frame", "BOTTOMRIGHT",
                        (0 - available_width) + (calc_dat["third_width"] * EngBags_BUTTONFRAME_WIDTH) +
                        (3 * EngBankConfig["frameXSpace"]),
                        cur_y,
