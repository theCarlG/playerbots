/// Ouro — Temple of Ahn'Qiraj. Burrows and emerges; while up it summons Ouro
/// Scarab (npc 15718) swarms. Clear the scarabs, then back to Ouro.
...
impl EncounterFsm for OuroFsm {
    ...
    fn phase_bt(&self, _fsm: crate::engine::macro_fsm::ActiveFsm) -> Option<Bt> {
        if self.active {
            Some(Sel!(Seq!(Bt::FocusNearestEntry(super::ENTRY_OURO_SCARAB))))
        } else {
            None
        }
    }
...
mod tests {
    ...
    fn focuses_add() {
        const ADD: u64 = 90;
        let mut fsm = OuroFsm::default();
        fsm.update(&EncounterEvent::CombatStarted, 1.0, 0);
        let bt = fsm
            .phase_bt(crate::engine::macro_fsm::ActiveFsm::Combat)
            .unwrap();
        let iface = MockWorld::new().with_nearby_entry(ADD, super::super::ENTRY_OURO_SCARAB);
