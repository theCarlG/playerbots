/// Fankriss the Unyielding — Temple of Ahn'Qiraj. Periodically teleports a
/// random raid member into a knot of adds and spawns worms: Spawn of Fankriss
/// (npc 15630) and Vekniss Hatchling (npc 15962). Clear the adds, then back to
/// the boss.
...
pub struct FankrissFsm {
    active: bool,
    done: bool,
}

impl EncounterFsm for FankrissFsm {
    ...
    fn boss_entry(&self) -> u32 {
        super::ENTRY_FANKRISS
    }
    fn phase_bt(&self, _fsm: crate::engine::macro_fsm::ActiveFsm) -> Option<Bt> {
        if self.active {
            Some(Sel!(
                Seq!(Bt::FocusNearestEntry(super::ENTRY_SPAWN_OF_FANKRISS)),
                Seq!(Bt::FocusNearestEntry(super::ENTRY_VEKNISS_HATCHLING)),
            ))
        } else {
            None
        }
...
mod tests {
    ...
    #[test]
    fn focuses_worm_adds() {
        const WORM: u64 = 90;
        let mut fsm = FankrissFsm::default();
        fsm.update(&EncounterEvent::CombatStarted, 1.0, 0);
        let bt = fsm
            .phase_bt(crate::engine::macro_fsm::ActiveFsm::Combat)
            .unwrap();
        let iface = MockWorld::new().with_nearby_entry(WORM, super::super::ENTRY_SPAWN_OF_FANKRISS);
        ...
        assert!(
            iface
                .events()
                .iter()
                .any(|e| matches!(e, MockEvent::Attack(h) if *h == WORM))
        );
    }
