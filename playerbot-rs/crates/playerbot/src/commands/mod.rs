...
use cmangos::{BotInventoryItem, ItemId, SpellId, UnitHandle};
...
fn resolve_spec_name(class: PlayerClass, name: &str) -> Option<PlayerSpec> {
    ...
    use PlayerSpec::{WarriorArms, WarriorFury, WarriorProtection, PaladinRetribution, PaladinHoly, PaladinProtection, PriestDiscipline, PriestShadow, PriestHoly, DruidBalance, DruidFeral, DruidRestoration, HunterBeastMastery, HunterMarksmanship, HunterSurvival, MageArcane, MageFire, MageFrost, RogueAssassination, RogueCombat, RogueSubtlety, ShamanElemental, ShamanEnhancement, ShamanRestoration, WarlockAffliction, WarlockDemonology, WarlockDestruction, DeathKnightBlood, DeathKnightUnholy, DeathKnightFrost};
    match (class, name) {
        // ── Warrior ──
...
fn format_item_link(item: &BotInventoryItem) -> String {
    ...
    let name = {
        // SAFETY: `item.name` is a fixed-size `[c_char; N]` whose bytes are
        // either valid UTF-8 followed by a NUL terminator or all zero. We
        // reinterpret the i8 buffer as u8 and stop at the first NUL.
        let bytes = unsafe {
            core::slice::from_raw_parts(item.name.as_ptr().cast::<u8>(), item.name.len())
        };
    ...
    match cmd {
        ...
        BotCommand::ListSkills => {
            ...
            let msg = if top.is_empty() {
                ...
                format!("Skills ({} total): {}", sorted.len(), top.join(", "))
            };
            reply(bot, pc, &msg);
        }
...
mod tests {
    ...
    /// Records each `tell_player` / `tell_addon` / `whisper` call so the
    /// reply-routing unit tests can assert which wire was used without a
    /// live FFI bridge. The recorder appends to two static `Mutex<Vec<…>>`
    /// sinks so tests can read them back after the boxed interface has been
    /// moved into `BotState`. Tests call `clear_reply_sinks()` at the start
    /// to isolate state (cargo-test runs cases on multiple threads, but
    /// each test scrubs+asserts the sinks under the mutex, which is
    /// sufficient for our single-assertion cases).
    struct ReplyRecorder {
        inner: cmangos::MockWorld,
    }
    ...
    // Forward the full trait surface to the inner `MockWorld` and only
    // override the three reply paths we want to observe. The forwarding
    // covers every non-default method on `World`; default methods inherit
    // their `World` defaults via the trait.
    impl World for ReplyRecorder {
        fn get_snapshot(&self) -> cmangos::BotWorldSnapshot {
            self.inner.get_snapshot()
        }
        fn get_unit_snapshot(&self, target: cmangos::UnitHandle) -> cmangos::BotUnitSnapshot {
            self.inner.get_unit_snapshot(target)
        }
        fn has_aura(&self, unit: cmangos::UnitHandle, spell_id: SpellId) -> bool {
            self.inner.has_aura(unit, spell_id)
        }
        fn get_aura(
            &self,
            unit: cmangos::UnitHandle,
            spell_id: SpellId,
        ) -> Option<cmangos::BotAuraInfo> {
            self.inner.get_aura(unit, spell_id)
        }
        fn get_auras(&self, unit: cmangos::UnitHandle) -> cmangos::AuraList<'_> {
            self.inner.get_auras(unit)
        }
        fn get_threat_list(
            &self,
            target_unit: cmangos::UnitHandle,
        ) -> cmangos::ThreatList<'_> {
            self.inner.get_threat_list(target_unit)
        }
        fn get_unit_threat(
            &self,
            target_unit: cmangos::UnitHandle,
            from_unit: cmangos::UnitHandle,
        ) -> f32 {
            self.inner.get_unit_threat(target_unit, from_unit)
        }
        fn unit_distance(&self, target: cmangos::UnitHandle) -> f32 {
            self.inner.unit_distance(target)
        }
        fn can_cast(&self, spell_id: SpellId, target: cmangos::UnitHandle) -> bool {
            self.inner.can_cast(spell_id, target)
        }
        fn spell_cooldown_ms(&self, spell_id: SpellId) -> u32 {
            self.inner.spell_cooldown_ms(spell_id)
        }
        fn has_los(&self, target: cmangos::UnitHandle) -> bool {
            self.inner.has_los(target)
        }
        fn get_nearby_units(&self, range: f32, hostile: bool) -> cmangos::UnitList<'_> {
            self.inner.get_nearby_units(range, hostile)
        }
        fn get_behind_position(
            &self,
            target: cmangos::UnitHandle,
            distance: f32,
        ) -> cmangos::BotPosition {
            self.inner.get_behind_position(target, distance)
        }
        fn get_safe_position(&self, search_radius: f32) -> Option<cmangos::BotPosition> {
            self.inner.get_safe_position(search_radius)
        }
        fn get_spread_position(
            &self,
            center: cmangos::UnitHandle,
            radius: f32,
            idx: u8,
            total: u8,
        ) -> cmangos::BotPosition {
            self.inner.get_spread_position(center, radius, idx, total)
        }
        fn can_reach(&self, x: f32, y: f32, z: f32) -> bool {
            self.inner.can_reach(x, y, z)
        }
        fn cast_spell(&self, spell_id: SpellId, target: cmangos::UnitHandle) -> bool {
            self.inner.cast_spell(spell_id, target)
        }
        fn cast_spell_pos(&self, spell_id: SpellId, x: f32, y: f32, z: f32) -> bool {
            self.inner.cast_spell_pos(spell_id, x, y, z)
        }
        fn move_to(&self, x: f32, y: f32, z: f32) -> bool {
            self.inner.move_to(x, y, z)
        }
        fn follow(&self, target: cmangos::UnitHandle, dist: f32, angle: f32) -> bool {
            self.inner.follow(target, dist, angle)
        }
        fn stop_moving(&self) -> bool {
            self.inner.stop_moving()
        }
        fn attack(&self, target: cmangos::UnitHandle) -> bool {
            self.inner.attack(target)
        }
        fn auto_attack(&self, enable: bool) -> bool {
            self.inner.auto_attack(enable)
        }
        fn say(&self, msg: &str, lang: u32) -> bool {
            self.inner.say(msg, lang)
        }
        fn use_item(&self, item_id: ItemId, target: cmangos::UnitHandle) -> bool {
            self.inner.use_item(item_id, target)
        }
        fn taunt(&self, target: cmangos::UnitHandle) -> bool {
            self.inner.taunt(target)
        }
        fn group_get_tank(&self) -> Option<cmangos::UnitHandle> {
            self.inner.group_get_tank()
        }
        fn group_get_healer(&self) -> Option<cmangos::UnitHandle> {
            self.inner.group_get_healer()
        }
        fn group_get_role(&self, member: cmangos::UnitHandle) -> BotRole {
            self.inner.group_get_role(member)
        }
    ...
    fn test_bot_with_recorder() -> BotState {
        BotState::new(
            ...
            Box::new(ReplyRecorder {
                inner: cmangos::MockWorld::default(),
            }),
            PlayerClass::Warrior,
            PlayerSpec::WarriorArms,
            BotRole::DPS,
